...
map< freenect_device *, ofxKinect *> kinects;
...
ofxKinect::ofxKinect()
{
	...
	bNeedsUpdate			= false;
	bUpdateTex				= false;
	bIsFrameNew = false; // new

	kinectContext			= NULL;
	kinectDevice			= NULL;
	
	targetTiltAngleDeg		= 0;
    currentTiltAngleDeg     = 0; //new
...
bool ofxKinect::open(int id){
	...
	if (freenect_open_device(kinectContext, &kinectDevice, id) < 0) {
		ofLog(OF_LOG_ERROR, "ofxKinect: Could not open device");
		return false;
	}

	freenect_set_user(kinectDevice, this);
	kinects[kinectDevice] = this;
	
	freenect_set_depth_callback(kinectDevice, &grabDepthFrame); //new
	freenect_set_video_callback(kinectDevice, &grabRgbFrame); // new
...
void ofxKinect::close(){
	...
	bIsFrameNew = false; //new
	bNeedsUpdate	= false; //new
	bUpdateTex		= false; //new
}
	// new start
//---------------------------------------------------------------------------
...
void ofxKinect::grabDepthFrame(freenect_device *dev, void *depth, uint32_t timestamp) {
	...
	if (kinects[dev]->lock()) {
		try {
			freenect_frame_mode curMode = freenect_get_current_depth_mode(dev); //new
			memcpy(kinects[dev]->depthPixelsBack, depth, FREENECT_DEPTH_11BIT);
			kinects[dev]->bNeedsUpdate = true;
		}
		catch(...) {
...
void ofxKinect::grabRgbFrame(freenect_device *dev, void *rgb, uint32_t timestamp) {
	...
	ofxKinect * thisKinect = kinects[dev];
	
	if (thisKinect->lock()) {
		try {
			freenect_frame_mode curMode = freenect_get_current_video_mode(dev); //new
			memcpy(thisKinect->videoPixelsBack, rgb, thisKinect->bInfrared?FREENECT_VIDEO_IR_8BIT:FREENECT_VIDEO_RGB);
...
void ofxKinect::threadedFunction(){	
	...
	freenect_start_depth(kinectDevice);
		// freenect_start_video(kinectDevice); //old
		// new start
	if(bGrabVideo) {
	...
	while (isThreadRunning()) {
        ...
        currentTiltAngleDeg = freenect_get_tilt_degs(tilt); //new
...
//TODO: uncomment these when they are implemented in freenect	
		//	freenect_set_tilt_degs(kinectDevice, 0);//old
		//	freenect_update_tilt_state(kinectDevice);//old
